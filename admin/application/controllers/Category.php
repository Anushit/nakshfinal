...
class Category extends MY_Controller {
	...
	public function datatable_json(){				   					   
		...
		{  
			...
			$action = "";
			if(@$this->general_user_premissions['category']['is_edit']==1){
				$action = '<a title="Edit" class="update btn btn-sm btn-warning" href="'.base_url('category/edit/'.$row['id']).'"> <i class="fa fa-pencil-square-o"></i></a>';
			}
			if(@$this->general_user_premissions['category']['is_delete']==1){
				$action .= '<a title="Delete" class="delete btn btn-sm btn-danger" href='.base_url("category/delete/".$row['id']).' title="Delete" onclick="return confirm(\'Do you want to delete ?\')"> <i class="fa fa-trash-o"></i></a>';
			}
			$data[]= array(
				++$i, 
				$row['name'],
	...
	public function add(){
		...
		if($this->input->post('submit')){
			...
			$this->form_validation->set_rules('sort_order', 'Sort Order', 'trim|strip_tags|xss_clean|required'); 
			$this->form_validation->set_rules('meta_title', 'Meta Title', 'trim|strip_tags|xss_clean|required');		 
			$this->form_validation->set_rules('is_feature', 'Is Featured', 'trim|strip_tags|xss_clean|required');
			$this->form_validation->set_rules('status', 'Status', 'trim|strip_tags|xss_clean|required');
			$this->form_validation->set_rules('slug', 'Seo URL', 'trim|strip_tags|xss_clean|required|is_unique[ci_categories.slug]');
	...
	public function edit($id = 0){
		...
		if($this->input->post('submit')){
			...
			if($this->input->post('slug') != $data['cat']['slug']) {
			   $uis_unique =  '|is_unique[ci_categories.slug]';
			} else {
			   $uis_unique =  '';
			}
			$this->form_validation->set_rules('parent_id', 'Parent Category', 'trim|strip_tags|xss_clean|required');
			$this->form_validation->set_rules('name', 'Name', 'trim|strip_tags|xss_clean|required');
			$this->form_validation->set_rules('sort_order', 'Sort Order', 'trim|strip_tags|xss_clean|required'); 
			$this->form_validation->set_rules('meta_title', 'Meta Title', 'trim|strip_tags|xss_clean|required');	
			$this->form_validation->set_rules('is_feature', 'Is Featured', 'trim|strip_tags|xss_clean|required');
			$this->form_validation->set_rules('status', 'Status', 'trim|strip_tags|xss_clean|required');
			$this->form_validation->set_rules('slug', 'Seo URL', 'trim|strip_tags|xss_clean|required'.$uis_unique);
	...
	{		
		$query = $this->db->get_where('ci_product_to_category', array('category_id' => $id));
		$resultcat = $query->row_array();

		$this->db->delete('ci_categories', array('id' => $id));
		$this->db->delete('ci_categories', array('parent_id' => $id));

		if(!empty($resultcat)){		
			$this->db->delete('ci_product_to_category', array('category_id' => $id));
			$this->db->delete('ci_products', array('id' => $resultcat['product_id']));
			$this->db->delete('ci_product_image', array('product_id' => $resultcat['product_id']));
		} 
