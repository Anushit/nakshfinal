...
class Career extends MY_Controller {
	...
	public function datatable_json(){				   					   
		...
		{  
			...
			$action = "";
			if(@$this->general_user_premissions['career']['is_edit']==1){
				$action = '<a title="View" class="view btn btn-sm btn-info" href="'.base_url('career/edit/'.$row['id']).'"> <i class="fa fa-pencil"></i></a>';
			}
			if(@$this->general_user_premissions['career']['is_delete']==1){
				$action .= '<a title="Delete" class="delete btn btn-sm btn-danger" href='.base_url("career/delete/".$row['id']).' title="Delete" onclick="return confirm(\'Do you want to delete ?\')"> <i class="fa fa-trash-o"></i></a>';
			}
			$data[]= array(
				++$i,
				$row['name'],
	...
	public function jobresult($id=NULL){   
		if($this->input->post('submit')){
			$this->form_validation->set_rules('job_result', 'Job Result', 'trim|strip_tags|xss_clean|required');
	...
	public function add(){
		...
		if($this->input->post('submit')){
			$this->form_validation->set_rules('name', 'Career Name', 'trim|strip_tags|xss_clean|required');
			$this->form_validation->set_rules('type', 'Career Type', 'trim|strip_tags|xss_clean|required');
			$this->form_validation->set_rules('opening_date', 'Opening date', 'trim|strip_tags|xss_clean|required');
			//$this->form_validation->set_rules('qualification', 'Qualification', 'trim|strip_tags|xss_clean|required');
			//$this->form_validation->set_rules('experince', 'Experince', 'trim|strip_tags|xss_clean|required'); 
			...
			$this->form_validation->set_rules('slug', 'Seo URL', 'trim|is_unique[ci_career.slug]');
	...
	public function edit($id = 0){
		...
		if($this->input->post('submit')){
			$olddata = $this->career_model->get_career_by_id($id);
			if($this->input->post('slug') != $olddata['slug']) {
			   $uis_unique =  '|is_unique[ci_career.slug]';
			} else {
			   $uis_unique =  '';
			}
			$this->form_validation->set_rules('name', 'Career Name', 'trim|strip_tags|xss_clean|required');
			...
			//$this->form_validation->set_rules('description', 'Description', 'trim|strip_tags|xss_clean|required'); 
			$this->form_validation->set_rules('status', 'Status', 'trim|strip_tags|xss_clean|required');
			$this->form_validation->set_rules('slug', 'Seo URL', 'trim|required'.$uis_unique);
	...
	{		
		$this->db->where('id', $id);
		$this->db->update('ci_career', array('deleted'=>1,'is_active'=>0));
		$this->session->set_flashdata('success', 'Career has been deleted successfully!');
		redirect(base_url('career'));
	}
}
