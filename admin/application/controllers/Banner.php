...
class Banner extends MY_Controller {
	...
	public function datatable_json(){				   					   
		...
		{  
			...
			$action = "";
				if(@$this->general_user_premissions['banner']['is_edit']==1){
					$action .= '<a title="Edit" class="update btn btn-sm btn-warning" href="'.base_url('banner/edit/'.$row['id']).'"> <i class="fa fa-pencil-square-o"></i></a>';
				}
				if(@$this->general_user_premissions['banner']['is_delete']==1){
					$action .= '<a title="Delete" class="delete btn btn-sm btn-danger" href='.base_url("banner/delete/".$row['id']).' title="Delete" onclick="return confirm(\'Do you want to delete ?\')"> <i class="fa fa-trash-o"></i></a>';
				}
			$data[]= array(
				++$i,
		...
		if($this->input->post('submit')){
			...
			if ($this->form_validation->run() == FALSE) {
				...
				if(!empty($_FILES['image']['name']))
				{
					...
					$result = $this->functions->file_insert($path, 'image', 'image', '9097152000000');
					if($result['status'] == 1){
	...
	{		
		...
		$this->db->where('id', $id);
		$this->db->update('ci_banners', array('deleted'=>1,'is_active'=>0));
