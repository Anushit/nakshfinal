...
class Product extends MY_Controller {
	...
	public function datatable_json(){				   					   
		...
		{  
			...
			$action = "";
			if(@$this->general_user_premissions['product']['is_edit']==1){
				$action .= '<a title="Edit" class="update btn btn-sm btn-warning" href="'.base_url('product/edit/'.$row['id']).'"> <i class="fa fa-pencil-square-o"></i></a>';
			}
			if(@$this->general_user_premissions['product']['is_delete']==1){
				$action .= '<a title="Delete" class="delete btn btn-sm btn-danger" href='.base_url("product/delete/".$row['id']).' title="Delete" onclick="return confirm(\'Do you want to delete ?\')"> <i class="fa fa-trash-o"></i></a>';
			}
			$data[]= array(
				++$i,
				$row['model'],
	...
	public function add(){
		...
		if($this->input->post('submit')){
			...
			//$this->form_validation->set_rules('demo_link', 'Demo Link', 'trim|required');
			...
			$this->form_validation->set_rules('slug', 'Seo URL', 'trim|required|is_unique[ci_products.slug]');
			//$this->form_validation->set_rules('amazon_link', 'amazon_link', 'trim|required');
			//$this->form_validation->set_rules('filpcart_link', 'filpcart_link', 'trim|required');
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$data['pro'] = array( 
					...
					'filpcart_link' => $this->input->post('filpcart_link'),
				);
	...
	public function edit($id = 0){
		...
		if($this->input->post('submit')){
			$olddata = $this->product_model->get_product_by_id($id);
			if($this->input->post('slug') != $olddata['slug']) {
			   $uis_unique =  '|is_unique[ci_products.slug]';
			} else {
			   $uis_unique =  '';
			}
			$this->form_validation->set_rules('category_id', 'Category', 'trim|required');
			...
			$data['pro'] = $this->product_model->get_product_by_id($id);
			$data['procategory'] = $this->product_model->get_product_category($id);
			$data['proimage'] = $this->product_model->get_product_image($id);
			$data['parcat']=$this->category_model->getallCategory();
	...
	{	
		...
		$this->db->where('id', $id);
		$this->db->update('ci_products', array('deleted'=>1,'is_active'=>0));
