...
class Cms extends MY_Controller {
	...
	public function datatable_json(){				   					   
		...
		{  
			...
			$action= "";
			if(@$this->general_user_premissions['cms']['is_edit']==1){
				$action .= '<a title="Edit" class="update btn btn-sm btn-warning" href="'.base_url('cms/edit/'.$row['id']).'"> <i class="fa fa-pencil-square-o"></i></a>';
			}
			/*if(@$this->general_user_premissions['cms']['is_delete']==1){
				$action .= '<a title="Delete" class="delete btn btn-sm btn-danger" href='.base_url("cms/delete/".$row['id']).' title="Delete" onclick="return confirm(\'Do you want to delete ?\')"> <i class="fa fa-trash-o"></i></a>';
			}*/
			$data[]= array(
				++$i,
				$row['cms_name'],
	...
	public function add(){
		...
		if($this->input->post('submit')){
			$this->form_validation->set_rules('cms_name', 'Cms Name', 'trim|strip_tags|xss_clean|required');
			$this->form_validation->set_rules('cms_title', 'Cms Title', 'trim|strip_tags|xss_clean|required');
			...
			$this->form_validation->set_rules('slug', 'Seo URL', 'trim|required|strip_tags|xss_clean|is_unique[ci_cms.slug]');
	...
	public function edit($id = 0){
		...
		if($this->input->post('submit')){
			if($this->input->post('slug') != $data['cms']['slug']) {
			   $uis_unique =  '|is_unique[ci_cms.slug]';
			} else {
			   $uis_unique =  '';
			}
	...
	{		
		$this->db->where('id', $id);
		$this->db->update('ci_cms', array('deleted'=>1,'is_active'=>0));
