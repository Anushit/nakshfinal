	...
	class Functions 
	{
		...
		*/
		...
		function file_insert($location, $controlname, $type, $size)
		{
			...
			if(isset($_FILES[$controlname]) && $_FILES[$controlname]['name'] != NULL)
	        {
				...
				elseif($type == 'image/video')
				{
					if($file_extension == 'jpg' || $file_extension == 'jpeg' || $file_extension == 'png' || $file_extension == 'gif' || $file_extension == 'mp4')
					{
						if ($filesize <= $size) 
						{
							$return['msg'] = $this->file_upload($location, $controlname);
					...
					{
						$return['msg'] = 'File Must Be In jpg,jpeg,png,gif,mp4 Format';
						$return['status'] = 0;
		...
		function file_insert_bulk($path, $filename, $filetempname, $file_extension, $filesize, $size){
			...
			if($file_extension == 'jpg' || $file_extension == 'jpeg' || $file_extension == 'png' || $file_extension == 'gif')
			{
				if ($filesize <= $size) 
				{
					...
					$new_filename= $this->rename_image($filename);
					if(move_uploaded_file(realpath($filetempname),$path.$new_filename))
					{
		...
		*/
		...
		{//print_r($location);die();
			...
			{//print_r(file_exists(FCPATH.$location));die();
