<?php
	class Product_model extends CI_Model{
		...
		public function get_products(){
			$query = $this->db->get_where('ci_products', array('is_active' => 1,'deleted'=>0));
		...
		public function getTotalProducts($data = array()) {
			$sql = "SELECT COUNT(p.id) AS total";
 			$sql .= " FROM ci_products as p"; 
			$sql .= " inner join ci_product_to_category as p2c on p2c.product_id=p.id"; 
			$sql .= " inner join ci_categories as c on c.id = p2c.category_id"; 
        ...
        public function getProducts($data = array()) {
			//print_r($data);die;
			$sql = "SELECT p.*,(select image from ci_product_image pi where pi.product_id=p.id order by sort_order ASC LIMIT 1 ) as image, c.id as category_id";
 			$sql .= " FROM ci_products p"; 
			$sql .= " left join ci_product_to_category p2c on p2c.product_id=p.id";
			$sql .= " left join ci_categories c on p2c.category_id=c.id";  
			$sql .= " where p.is_active=1 ";
			if (!empty($data['category_id'])) {
				$sql .= " AND c.slug = '" . $data['category_id'] . "'"; 
			}  
			if (($data['sort'] != '')) { 
				$sql .= " ORDER BY " . $data['sort'] . " " . $data['order'];
			}
