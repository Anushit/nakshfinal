<?php
	class Gallery_model extends CI_Model{
		...
		function get_cat_with_data($type=Null)
		{

			
			if($type=='video')
			{
				$sql = "SELECT ci_gallery.id,ci_gallery.album FROM ci_gallery JOIN ci_gallery_details ON ci_gallery_details.gallery_id=ci_gallery.id WHERE ci_gallery.type=2  GROUP BY ci_gallery_details.gallery_id";

				
			}
			else
			{
				$sql = "SELECT ci_gallery.id,ci_gallery.album FROM ci_gallery JOIN ci_gallery_details ON ci_gallery_details.gallery_id=ci_gallery.id WHERE ci_gallery.type=1  GROUP BY ci_gallery_details.gallery_id";
			}
