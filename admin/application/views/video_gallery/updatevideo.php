                    ...
                    <tbody> 
                      <?php foreach ($proimage as $ikey => $ivalue) {  
                        $htmlData = '<tr id="image-row'.$image_row.'"><td><div class="row">';   
                          ...
                          $htmlData.= '</div></td><td><input type="text" name="img_order['.$image_row.']" class="form-control" id="img_order['.$image_row.']" placeholder="" onkeyup="if (/\D/g.test(this.value)) this.value = this.value.replace(/\D/g, \' \')" value="'.@$ivalue['sort_order'].'"> <input type="hidden" name="video_url['.$image_row.']" value="'.html_escape($ivalue['value']).'"></td><td><span class="removeimg" cid="'.$image_row.'"><i class="fa fa-trash"></i></span></td></tr>';
                          $image_row++;
                          echo $htmlData; 
                      } ?>
                    </tbody>
